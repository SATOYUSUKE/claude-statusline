...
# Cache hit rate (current turn: cache_read / all input tokens)
cache_pct=0
total_cur=$(( cache_read + cache_creation + 1 ))
if (( total_cur > 0 )); then
  cache_pct=$(( cache_read * 100 / total_cur ))
fi
...
line1="📁 ${folder}${sep}🤖 ${model}${sep}${YELLOW}${cost_str}${RESET}${sep}✏️ +${lines_added}/-${lines_removed}${sep}🔀 ${git_branch}"

# ── Line 2: Context detail ──
ctx_alert=""
if [ "$exceeds_200k" = "true" ]; then
  ctx_alert="  ${RED}⚠ >200k${RESET}"
fi
line_ctx="${ctx_color}🧠 ${ctx_int}%${RESET}${sep}IN:$(fmt_k "$in_tokens")  OUT:$(fmt_k "$out_tokens")  cache:${cache_pct}%${ctx_alert}"
